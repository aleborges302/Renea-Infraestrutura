...
// Importa ativos a partir de um CSV (mesmo formato do Exportar, separado por ';').
// Mínimo necessário: colunas Nome, Categoria e Localização.
async function importCSV(event) {
  ...
  try {
    ...
    if (linhas.length < 2) { showToast('CSV vazio ou sem dados.', '#d03b3b'); return; }

    const header = parseLine(linhas[0]).map(h => h.toLowerCase());
    const idx = (...names) => { for (const n of names) { const i = header.indexOf(n); if (i >= 0) return i; } return -1; };
    const iCod = idx('código','codigo'), iNome = idx('nome'), iCat = idx('categoria'),
          iUser = idx('usuário','usuario'), iLoc = idx('localização','localizacao'),
          iSerie = idx('nº série','no série','serie','série'), iFab = idx('fabricante'), iStatus = idx('status');

    if (iNome < 0 || iCat < 0 || iLoc < 0) {
      showToast('O CSV precisa ter ao menos as colunas: Nome, Categoria, Localização.', '#d03b3b'); return;
    }

    const statusCode = (s = '') => { s = s.toLowerCase(); if (s.startsWith('atenç') || s === 'warn') return 'warn'; if (s.startsWith('inat') || s === 'err') return 'err'; return 'ok'; };
    let maxNum = inventario.reduce((m, r) => Math.max(m, parseInt((r.codigo || '').replace('IT-','')) || 0), 0);
    const existentes = new Set(inventario.map(r => r.codigo));

    const novos = [];
    for (let i = 1; i < linhas.length; i++) {
      const c = parseLine(linhas[i]);
      const nome = (c[iNome] || '').trim();
      if (!nome) continue;
      let codigo = iCod >= 0 ? (c[iCod] || '').trim() : '';
      if (!codigo || existentes.has(codigo)) codigo = 'IT-' + String(++maxNum).padStart(4, '0');
      existentes.add(codigo);
      novos.push({
        codigo, nome,
        categoria:    (c[iCat] || '').trim() || 'Outro',
        localizacao:  (c[iLoc] || '').trim() || 'Outro',
        usuario_resp: iUser  >= 0 ? (c[iUser]  || '').trim() || null : null,
        serie:        iSerie >= 0 ? (c[iSerie] || '').trim() || null : null,
        fabricante:   iFab   >= 0 ? (c[iFab]   || '').trim() || null : null,
        status:       iStatus >= 0 ? statusCode(c[iStatus]) : 'ok',
        created_by:   currentUser.id,
      });
    }
    if (!novos.length) { showToast('Nenhuma linha válida encontrada.', '#d03b3b'); return; }

    const { data, error } = await sb.from('ativos').insert(novos).select();
    if (error) { showToast('Erro ao importar: ' + error.message, '#d03b3b'); return; }

    await sb.from('historico').insert(data.map(a => ({
      ativo_id: a.id, ativo_codigo: a.codigo, ativo_nome: a.nome,
      tipo: 'Cadastro', descricao: `Importado via CSV por ${currentProfile.nome}.`,
      responsavel: currentProfile.nome, created_by: currentUser.id,
    })));

    await loadAtivos(); await loadHistorico(); renderAlertas();
    showToast(`${data.length} ativo(s) importado(s) com sucesso!`);
  } catch (err) {
    showToast('Falha ao ler o arquivo: ' + err.message, '#d03b3b');
  } finally {
    event.target.value = '';
  }
}
