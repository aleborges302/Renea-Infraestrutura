...
function renderNotebooks(data) {
  ...
  body.innerHTML = data.map(n => `
    <tr>
      ...
      <td>
        <div class="row-actions" style="display:flex;gap:6px;justify-content:flex-end;flex-wrap:nowrap">
          <button class="icon-btn" title="Editar" onclick="event.stopPropagation();editNotebook('${n.id}')"><i class="ti ti-pencil" style="font-size:15px"></i></button>
          <button class="icon-btn" title="Gerar Termo" onclick="event.stopPropagation();gerarTermoNotebook('${n.id}')"><i class="ti ti-file-text" style="font-size:15px"></i></button>
          <button class="icon-btn danger" title="Excluir" onclick="event.stopPropagation();deleteNotebook('${n.id}','${(n.modelo||'').replace(/'/g,'')}')"><i class="ti ti-trash" style="font-size:15px"></i></button>
        </div>
      </td>
    </tr>`).join('');
}
...
async function salvarNotebook() {
  ...
  const fileInput = document.getElementById('nb-foto');
  const file = fileInput ? fileInput.files[0] : null;
  if (file) {
    const okTypes = ['image/jpeg','image/png','image/webp','image/gif'];
    if (!okTypes.includes(file.type)) { showToast('Foto deve ser JPG, PNG ou WEBP.', '#d03b3b'); return; }
    if (file.size > 10 * 1024 * 1024) { showToast('Foto acima de 10 MB.', '#d03b3b'); return; }
    const safeName = file.name.replace(/[^\w.\-]/g, '_');
    const path = `notebooks/${Date.now()}_${safeName}`;
    const { error: upErr } = await sb.storage.from('anexos').upload(path, file, { upsert: false });
    if (upErr) { showToast('Erro no upload: ' + upErr.message, '#d03b3b'); return; }
    foto_url  = sb.storage.from('anexos').getPublicUrl(path).data.publicUrl;
    foto_nome = file.name;
  }
...
function resetNotebookModal() {
  ...
  ['nb-modelo','nb-fab','nb-serie','nb-locadora','nb-loc','nb-user','nb-custo','nb-contrato','nb-data-ini','nb-obs'].forEach(id => {
...
function gerarTermo(id) {
  ...
  const isImagem = a.anexo_url && /\.(jpe?g|png|gif|webp)(\?|$)/i.test(a.anexo_url);
  const fotoHtml = isImagem ? `
    <div style="margin-bottom:16px">
      <p class="section-title">Foto do equipamento:</p>
      <img src="${a.anexo_url}" alt="Foto do equipamento"
        style="max-width:260px;max-height:180px;object-fit:contain;border:1px solid #ccc;border-radius:6px;padding:4px;display:block">
    </div>` : '';
...
<div class="header-box">
  ...
  <div class="info-row">
    <div class="info-cell">CPF: ___________________________</div>
...
function gerarTermoNotebook(id) {
  ...
  const isImagem = n.foto_url && /\.(jpe?g|png|gif|webp)(\?|$)/i.test(n.foto_url);
  const fotoHtml = isImagem ? `
    <div style="margin-bottom:16px">
      <p style="font-weight:bold;margin:14px 0 8px;font-size:12px">Foto do equipamento:</p>
      <img src="${n.foto_url}" alt="Foto do equipamento"
        style="max-width:260px;max-height:180px;object-fit:contain;border:1px solid #ccc;border-radius:6px;padding:4px;display:block">
    </div>` : '';
...
<table>
  <thead>
    <tr><th>ITEM</th><th>EQUIPAMENTO</th><th>MARCA/MODELO</th><th>S/N</th><th>LOCADORA</th><th>DATA</th></tr>
  </thead>
  <tbody>
    <tr>
      <td style="text-align:center">1</td>
