...
function renderNotebooks(data) {
  ...
  const p = PERMS[currentProfile.role];
  if (!data.length) {
    body.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:32px;color:var(--text-muted)">Nenhum notebook locado cadastrado.</td></tr>`;
    return;
  }
  const fmt = v => v != null ? 'R$ ' + parseFloat(v).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) : '—';
  ...
  body.innerHTML = data.map(n => `
    <tr>
      ...
      <td><div class="row-actions">
        ${p.canEdit ? `<button class="icon-btn" title="Editar" onclick="editNotebook('${n.id}')"><i class="ti ti-pencil" style="font-size:15px"></i></button>` : ''}
        <button class="icon-btn" title="Gerar Termo" onclick="gerarTermoNotebook('${n.id}')"><i class="ti ti-file-text" style="font-size:15px"></i></button>
        ${p.canDelete ? `<button class="icon-btn danger" title="Excluir" onclick="deleteNotebook('${n.id}','${(n.modelo||'').replace(/'/g,'')}')"><i class="ti ti-trash" style="font-size:15px"></i></button>` : ''}
      </div></td>
    </tr>`).join('');
}
