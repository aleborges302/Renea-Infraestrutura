...
function editNotebook(id) {
  ...
  if (!n) return;
  editNotebookId = id;
  document.getElementById('nb-titulo').childNodes[0].nodeValue = 'Editar notebook locado ';
...
function resetNotebookModal() {
  editNotebookId = null;
  document.getElementById('nb-titulo').childNodes[0].nodeValue = 'Novo notebook locado ';
  ['nb-modelo','nb-fab','nb-serie','nb-locadora','nb-loc','nb-user','nb-custo','nb-contrato','nb-data-ini','nb-obs'].forEach(id => document.getElementById(id).value = '');
...
function gerarTermoNotebook(id) {
  const n = notebooksLocados.find(r => r.id === id);
  if (!n) return;
  // Monta um objeto compatível com gerarTermo
  const ativo = {
    id: n.id,
    usuario_resp: n.usuario_resp,
    categoria: 'Notebook',
