...
function applyFilters() {
  ...
  const filtered = inventario.filter(r =>
    (!q   || r.nome.toLowerCase().includes(q) || r.codigo.toLowerCase().includes(q) || (r.patrimonio||'').toLowerCase().includes(q) || (r.usuario_resp||'').toLowerCase().includes(q) || (r.setor||'').toLowerCase().includes(q)) &&
    (!cat || r.categoria === cat) &&
...
function renderInventory(data) {
  ...
  if (!data.length) {
    body.innerHTML = `<tr><td colspan="${canDel ? 10 : 9}" style="text-align:center;padding:32px;color:var(--text-muted)">Nenhum ativo encontrado.</td></tr>`;
    ...
    return `
      ...
      <tr id="${grpId}" class="grp-body">
        <td colspan="${cols}" style="padding:0">
          <table class="full-table" style="margin:0">${linhas}</table>
        </td>
      </tr>`;
  }).join('');
...
function resetNovoModal() {
  ...
  ['f-nome','f-serie','f-fab','f-user','f-data','f-valor','f-garantia','f-obs','f-modelo','f-setor','f-custo','f-patrimonio'].forEach(id => document.getElementById(id).value = '');
...
async function devolverAoEstoque(ativoId) {
  ...
  // 2) Remove só o ativo (mantém o histórico no banco para auditoria futura)
  const { error: errAtivo } = await sb.from('ativos').delete().eq('id', ativoId);
  if (errAtivo) { showToast('Item criado no estoque, mas erro ao remover do inventário: ' + errAtivo.message, '#d03b3b'); return; }
