...
let manutencoes = [];
let estoque = [];
let fornecedores = []; // mantido para compatibilidade com alertas
let auditoria = [];
let usuarios = [];
...
async function showApp() {
  ...
  document.getElementById('btn-novo-estoque').style.display = p.canCreate ? 'inline-flex' : 'none';
  ...
  await Promise.all([loadAtivos(), loadHistorico(), loadLicencas(), loadImpressoras(),
                     loadNotebooksLocados(), loadChamados(), loadManutencoes(), loadEstoque(), loadKPIs()]);
  if (p.verSenhas) loadCredenciais();
  if (p.verAuditoria) loadAuditoria();
  if (p.verUsuarios) loadUsuarios();
  initCharts();
  renderAlertas();
}
...
function populateFilterOptions() {
  ...
  const ccs = [...new Set(inventario.map(r => r.centro_custo).filter(Boolean))].sort();
  const dlCC = document.getElementById('dl-cc');
  if (dlCC) dlCC.innerHTML = ccs.map(c => `<option>${c}</option>`).join('');
  const fCC = document.getElementById('filterCC');
  if (fCC) { const keep = fCC.value; fCC.innerHTML = '<option value="">Todos os centros de custo</option>' + ccs.map(c => `<option>${c}</option>`).join(''); fCC.value = keep; }
...
function applyFilters() {
  ...
  const filtered = inventario.filter(r =>
    (!q   || r.nome.toLowerCase().includes(q) || r.codigo.toLowerCase().includes(q) || (r.patrimonio||'').toLowerCase().includes(q) || (r.usuario_resp||'').toLowerCase().includes(q) || (r.setor||'').toLowerCase().includes(q) || (r.numero_corporativo||'').toLowerCase().includes(q)) &&
    (!cat || r.categoria === cat) &&
    (!loc || r.localizacao === loc) &&
    (!setor || r.setor === setor) &&
    (!cc || r.centro_custo === cc) &&
    (!st  || r.status === st)
  );
...
function clearFilters() {
  ['searchInput','filterCat','filterLoc','filterSetor','filterCC','filterStatus'].forEach(id => document.getElementById(id).value = '');
  applyFilters();
}
...
function renderInventory(data) {
  ...
  if (!data.length) {
    body.innerHTML = `<tr><td colspan="${canDel ? 11 : 10}" style="text-align:center;padding:32px;color:var(--text-muted)">Nenhum ativo encontrado.</td></tr>`;
  ...
  const grupos = {};
  data.forEach(r => {
    const u = r.usuario_resp || 'Sem responsável';
    if (!grupos[u]) grupos[u] = [];
    grupos[u].push(r);
  });

  // Ordena: usuários com nome primeiro, "Sem responsável" por último
  ...
  const cols = canDel ? 11 : 10;

  body.innerHTML = keys.map(usuario => {
    const itens = grupos[usuario];
    const grpId = 'grp-' + usuario.replace(/[^a-z0-9]/gi, '_');
    const totalCusto = itens.reduce((s, r) => s + (r.custo_mensal || 0), 0);
    const custoStr = totalCusto > 0 ? ` · R$ ${totalCusto.toLocaleString('pt-BR', {minimumFractionDigits:2})}/mês` : '';

    const linhas = itens.map(r => `
      <tr onclick="showDetalhe('${r.id}')" style="cursor:pointer">
        ${canDel ? `<td style="width:32px" onclick="event.stopPropagation()"><input type="checkbox" class="sel-chk" ${selecionados.has(String(r.id)) ? 'checked' : ''} onchange="toggleSel('${r.id}',this)"></td>` : ''}
        <td style="width:100px;color:var(--text-muted);font-size:12px">${r.patrimonio || '—'}</td>
        <td style="font-weight:500;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.nome}</td>
        <td style="width:110px;color:var(--text-secondary)">${r.categoria}</td>
        <td style="width:100px;color:var(--text-secondary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.setor || '—'}</td>
        <td style="width:120px;color:var(--text-secondary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.centro_custo || '—'}</td>
        <td style="width:120px;color:var(--text-secondary);font-size:12px">${r.numero_corporativo || '—'}</td>
        <td style="width:150px;color:var(--text-secondary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.localizacao}</td>
        <td style="width:85px">${statusLabel[r.status]}</td>
        <td style="width:48px"><button class="action-btn" onclick="event.stopPropagation();showDetalhe('${r.id}')">
          <i class="ti ti-eye" style="font-size:12px"></i>
        </button></td>
      </tr>`).join('');

    return `
      <tr class="grp-header" onclick="toggleGrupo('${grpId}')" style="cursor:pointer;background:var(--surface-1);user-select:none">
        <td colspan="${cols}" style="padding:10px 12px">
          <div style="display:flex;align-items:center;gap:10px">
            <i class="ti ti-chevron-down" id="ico-${grpId}" style="font-size:14px;color:var(--text-muted);transition:transform .2s"></i>
            <span style="font-weight:600;color:var(--text-primary)">${usuario}</span>
            <span style="font-size:12px;color:var(--text-muted)">${itens.length} item${itens.length > 1 ? 's' : ''}${custoStr}</span>
          </div>
        </td>
      </tr>
      <tr id="${grpId}" class="grp-body">
        <td colspan="${cols}" style="padding:0">
          <table class="full-table" style="width:100%">${linhas}</table>
        </td>
      </tr>`;
  }).join('');
...
function toggleGrupo(grpId) {
  ...
  const ico = document.getElementById('ico-' + grpId);
  if (!row) return;
...
async function salvarAtivo() {
  ...
  // Valida patrimônio único
  const patrimonioVal = document.getElementById('f-patrimonio').value.trim();
  if (patrimonioVal) {
    const duplicado = inventario.find(r =>
      r.patrimonio && r.patrimonio.trim() === patrimonioVal &&
      (!editing || String(r.id) !== String(editAtivoId))
    );
    if (duplicado) {
      showToast(`Patrimônio "${patrimonioVal}" já está cadastrado no ativo "${duplicado.nome}" (${duplicado.codigo}).`, '#d03b3b');
      document.getElementById('f-patrimonio').focus();
      return;
    }
  }
  ...
  const payload = {
    ...
    centro_custo: document.getElementById('f-cc').value || null,
    modelo:       document.getElementById('f-modelo').value || null,
    usuario_resp: document.getElementById('f-user').value || null,
    numero_corporativo: document.getElementById('f-numcorp').value || null,
    serie:        document.getElementById('f-serie').value || null,
    patrimonio:   document.getElementById('f-patrimonio').value || null,
  ...
  if (editing) {
    ...
    // Compara campos e monta descrição detalhada das mudanças
    const campoLabel = {
      nome: 'Nome', categoria: 'Categoria', localizacao: 'Localização',
      setor: 'Setor', centro_custo: 'Centro de custo', modelo: 'Modelo',
      usuario_resp: 'Usuário', serie: 'Nº série', fabricante: 'Fabricante',
      patrimonio: 'Patrimônio', status: 'Status', observacoes: 'Observações',
      valor: 'Valor', custo_mensal: 'Custo mensal', garantia_ate: 'Garantia até',
      data_aquisicao: 'Data aquisição',
    };
    const diffs = [];
    Object.keys(campoLabel).forEach(k => {
      const ant = String(existente[k] ?? '').trim();
      const nov = String(payload[k] ?? '').trim();
      if (ant !== nov) diffs.push(`${campoLabel[k]}: "${ant || '—'}" → "${nov || '—'}"`);
    });
    // Compara softwares
    const swAnt = JSON.stringify(existente.softwares || {});
    const swNov = JSON.stringify(payload.softwares || {});
    if (swAnt !== swNov) diffs.push('Softwares/custos alterados');

    const descricao = diffs.length
      ? `Atualizado por ${currentProfile.nome}:\n${diffs.join('\n')}`
      : `Atualizado por ${currentProfile.nome} (sem alterações detectadas).`;

    await sb.from('historico').insert({
      ativo_id: editAtivoId, ativo_codigo: codigo, ativo_nome: nome,
      tipo: 'Atualização', descricao,
...
function resetNovoModal() {
  ...
  ['f-nome','f-serie','f-fab','f-user','f-numcorp','f-data','f-valor','f-garantia','f-obs','f-modelo','f-setor','f-custo','f-patrimonio'].forEach(id => document.getElementById(id).value = '');
  ['f-cat','f-loc','f-cc'].forEach(id => document.getElementById(id).value = '');
...
function editAtivo(id) {
  ...
  document.getElementById('f-cc').value       = a.centro_custo || '';
  document.getElementById('f-user').value     = a.usuario_resp || '';
  document.getElementById('f-numcorp').value  = a.numero_corporativo || '';
...
async function showDetalhe(id) {
  ...
  document.getElementById('detalhe-content').innerHTML = `
    ...
    <div class="info-grid">
      ...
      <div class="info-item"><div class="info-item-label">Centro de custo</div><div class="info-item-val">${a.centro_custo||'—'}</div></div>
      ...
      <div class="info-item"><div class="info-item-label">Nº corporativo</div><div class="info-item-val">${a.numero_corporativo||'—'}</div></div>
      ...
      <div class="info-item"><div class="info-item-label">Patrimônio</div><div class="info-item-val">${a.patrimonio||'—'}</div></div>
    ...
    ${hist && hist.length ? `
      ...
      <div class="timeline">${hist.map(h => `
        <div class="tl-item">
          ...
          <div class="tl-desc">${(h.descricao || '').split('\n').map((l,i) => i===0 ? l : `<span style="display:block;font-size:11px;color:var(--text-muted);margin-top:2px">• ${l}</span>`).join('')}</div>
    ...
    <div style="display:flex;gap:8px;margin-top:12px;padding-top:12px;border-top:0.5px solid var(--border);flex-wrap:wrap">
      ...
      ${p.canCreate ? `<button class="action-btn" onclick="devolverAoEstoque('${a.id}')">
        <i class="ti ti-arrow-left" style="font-size:12px"></i> Devolver ao estoque
      </button>` : ''}
    ...
    <tr>
      ...
      <td>
        <div style="display:flex;gap:6px;justify-content:flex-end;flex-wrap:nowrap">
          ...
          ${n.termo_url ? `<a href="${n.termo_url}" target="_blank" rel="noopener" style="background:none;border:0.5px solid var(--border);cursor:pointer;color:var(--success,#0ca30c);width:30px;height:30px;display:inline-flex;align-items:center;justify-content:center;border-radius:6px;text-decoration:none" title="Ver termo assinado" onclick="event.stopPropagation()"><i class="ti ti-file-check" style="font-size:15px"></i></a>` : ''}
...
function editNotebook(id) {
  ...
  loadSoftwareEditorNb(n.softwares);
  const termoInput = document.getElementById('nb-termo');
  if (termoInput) termoInput.value = '';
  const termoLink = document.getElementById('nb-termo-atual');
  if (termoLink) {
    if (n.termo_url) { termoLink.href = n.termo_url; termoLink.style.display = 'inline-flex'; termoLink.innerHTML = `<i class="ti ti-file-check" style="font-size:13px"></i> Ver termo atual${n.termo_nome ? ' (' + n.termo_nome + ')' : ''}`; }
    else { termoLink.style.display = 'none'; }
  }
  openModal('Notebook');
}
...
async function salvarNotebook() {
  ...
  let foto2_url  = editing ? (existente.foto2_url  || null) : null;
  let foto2_nome = editing ? (existente.foto2_nome || null) : null;
  let termo_url  = editing ? (existente.termo_url  || null) : null;
  let termo_nome = editing ? (existente.termo_nome || null) : null;
  ...
  const uploadTermo = async () => {
    const el = document.getElementById('nb-termo');
    const file = el ? el.files[0] : null;
    if (!file) return null;
    const okExt = /\.(pdf|jpe?g|png)$/i.test(file.name);
    const okTypes = ['application/pdf','image/jpeg','image/png'];
    if (!okTypes.includes(file.type) && !okExt) { showToast('Termo deve ser PDF, JPG ou PNG.', '#d03b3b'); throw new Error('tipo'); }
    if (file.size > 15 * 1024 * 1024) { showToast('Termo acima de 15 MB.', '#d03b3b'); throw new Error('tamanho'); }
    const safeName = file.name.replace(/[^\w.\-]/g, '_');
    const path = `notebooks/termo_${Date.now()}_${safeName}`;
    const { error: upErr } = await sb.storage.from('anexos').upload(path, file, { upsert: false });
    if (upErr) { showToast('Erro no upload do termo: ' + upErr.message, '#d03b3b'); throw new Error('upload'); }
    return { url: sb.storage.from('anexos').getPublicUrl(path).data.publicUrl, nome: file.name };
  };
  ...
  try {
    ...
    const t = await uploadTermo();
    if (t) { termo_url = t.url; termo_nome = t.nome; }
  } catch (e) { return; }
  ...
  const payload = {
    ...
    custo_mensal:  (function(){ const s = collectSoftwaresNb(); return s ? Object.values(s).reduce((a,b)=>a+b,0) : null; })(),
    softwares:     collectSoftwaresNb(),
...
function resetNotebookModal() {
  ...
  loadSoftwareEditorNb(null);
  const termoInput = document.getElementById('nb-termo');
  if (termoInput) termoInput.value = '';
  const termoLink = document.getElementById('nb-termo-atual');
  if (termoLink) termoLink.style.display = 'none';
}
...
function renderChamados(data) {
  ...
  const p = PERMS[currentProfile.role];
  if (!data.length) { body.innerHTML = `<tr><td colspan="9" style="text-align:center;padding:32px;color:var(--text-muted)">Nenhum chamado.</td></tr>`; return; }
  body.innerHTML = data.map(c => `
    <tr>
      <td style="color:var(--text-muted);font-size:12px">#${c.numero ?? '—'}</td>
      <td style="font-weight:500">${c.titulo}</td>
      <td>${c.solicitante || '—'}</td>
      <td style="color:var(--text-secondary);font-size:12px">${c.local || '—'}</td>
      <td style="color:var(--text-secondary);font-size:12px">${c.departamento || '—'}</td>
      <td>${c.responsavel || '—'}</td>
...
function editChamado(id) {
  ...
  document.getElementById('ch-local').value        = c.local || '';
  document.getElementById('ch-departamento').value = c.departamento || '';
...
async function salvarChamado() {
  ...
  const payload = {
    ...
    local:       document.getElementById('ch-local').value || null,
    departamento: document.getElementById('ch-departamento').value || null,
...
function resetChamModal() {
  ...
  ['ch-titulo','ch-ativo','ch-solicitante','ch-responsavel','ch-desc','ch-local','ch-departamento'].forEach(id => document.getElementById(id).value = '');
...
async function callAdmin(payload) {
  ...
  if (error) {
    ...
    if (/Failed to send|not found|404/i.test(msg)) msg = 'A função "admin-users" não foi encontrada. Verifique se ela foi publicada no Supabase.';
    throw new Error(msg);
  }
...
async function loadUsuarios() {
  ...
  body.innerHTML = `<tr><td colspan="5" style="text-align:center;padding:24px;color:var(--text-muted)">Carregando...</td></tr>`;
  try {
    const data = await callAdmin({ action: 'list' });
    usuarios = (data.users || []).sort((a, b) => (a.nome || a.email || '').localeCompare(b.nome || b.email || ''));
    renderUsers();
  } catch (e) {
    body.innerHTML = `<tr><td colspan="5" style="text-align:center;padding:24px;color:var(--danger)">${e.message}</td></tr>`;
  }
}

const roleLabel = { admin: 'Admin', tecnico: 'Técnico', auditor: 'Auditor' };
function renderUsers() {
  const q = (document.getElementById('userSearch').value || '').toLowerCase();
  const body = document.getElementById('usersBody');
  const list = usuarios.filter(u => !q || (u.nome || '').toLowerCase().includes(q) || (u.email || '').toLowerCase().includes(q));
  if (!list.length) { body.innerHTML = `<tr><td colspan="5" style="text-align:center;padding:24px;color:var(--text-muted)">Nenhum usuário.</td></tr>`; return; }
  body.innerHTML = list.map(u => {
    const eu = currentUser && u.id === currentUser.id;
    const opts = ['tecnico', 'admin', 'auditor'].map(r => `<option value="${r}" ${u.role === r ? 'selected' : ''}>${roleLabel[r]}</option>`).join('');
    const acesso = u.last_sign_in_at ? new Date(u.last_sign_in_at).toLocaleString('pt-BR') : 'nunca';
    return `<tr>
      <td style="font-weight:500">${u.nome || '—'}${eu ? ' <span style="font-size:10px;color:var(--text-muted)">(você)</span>' : ''}</td>
      <td style="color:var(--text-secondary)">${u.email || '—'}</td>
      <td><select onchange="mudarPapel('${u.id}',this.value)" style="height:30px;border:0.5px solid var(--border);border-radius:6px;padding:0 8px;font-size:12px;background:var(--surface-2)">${opts}</select></td>
      <td style="font-size:12px;color:var(--text-muted)">${acesso}</td>
      <td><div class="row-actions">
        <button class="icon-btn" title="Resetar senha" onclick="resetarSenha('${u.id}','${(u.email || '').replace(/'/g, '')}')"><i class="ti ti-key" style="font-size:15px"></i></button>
        ${eu ? '' : `<button class="icon-btn danger" title="Excluir usuário" onclick="excluirUsuario('${u.id}','${(u.email || '').replace(/'/g, '')}')"><i class="ti ti-trash" style="font-size:15px"></i></button>`}
      </div></td>
    </tr>`;
  }).join('');
}
...
async function salvarUsuario() {
  ...
  if (!email || !senha) { showToast('Preencha e-mail e senha.', '#d03b3b'); return; }
  try {
    await callAdmin({ action: 'create', email, password: senha, nome, role });
    closeModal('Usuario');
    ['u-email', 'u-senha', 'u-nome'].forEach(id => document.getElementById(id).value = '');
    document.getElementById('u-role').value = 'tecnico';
    await loadUsuarios();
    showToast('Usuário criado com sucesso!');
  } catch (e) { showToast('Erro: ' + e.message, '#d03b3b'); }
}

async function mudarPapel(id, role) {
  try { await callAdmin({ action: 'setRole', id, role }); const u = usuarios.find(x => x.id === id); if (u) u.role = role; showToast('Perfil atualizado.'); }
  catch (e) { showToast('Erro: ' + e.message, '#d03b3b'); loadUsuarios(); }
}

async function resetarSenha(id, email) {
  const nova = prompt(`Nova senha para ${email} (mínimo 6 caracteres):`);
  if (nova === null) return;
  if (nova.length < 6) { showToast('Senha muito curta.', '#d03b3b'); return; }
  try { await callAdmin({ action: 'resetPassword', id, password: nova }); showToast('Senha redefinida.'); }
  catch (e) { showToast('Erro: ' + e.message, '#d03b3b'); }
}

async function excluirUsuario(id, email) {
  if (!confirm(`Excluir o usuário ${email}? Esta ação é irreversível.`)) return;
  try { await callAdmin({ action: 'delete', id }); await loadUsuarios(); showToast('Usuário excluído.'); }
  catch (e) { showToast('Erro: ' + e.message, '#d03b3b'); }
}
...
async function importPlanilha(event) {
  ...
  try {
    ...
    const col = {
      ...
      cc:      find(h => h.includes('centro') && h.includes('custo')),
      patrim:  find(h => h.includes('patrim')),
    ...
    for (let r = hIdx + 1; r < matrix.length; r++) {
      ...
      novos.push({
        ...
        centro_custo: get(col.cc) || null,
        patrimonio:   get(col.patrim) || null,
        modelo:       modelo || null,
...
function updateDashboard() {
  ...
  // ── Licenças ──
  const licComTotal = licencas.filter(l => l.total > 0);
  const totalLic = licComTotal.reduce((s, l) => s + (l.total || 0), 0);
  const usadasLic = licComTotal.reduce((s, l) => s + (l.usadas || 0), 0);
  const pctLic = totalLic ? Math.round(usadasLic / totalLic * 100) : 0;
  const elPct = document.getElementById('kpi-lic-pct');
  const elSub = document.getElementById('kpi-lic-sub');
  const elBar = document.getElementById('kpi-lic-bar');
  if (elPct) elPct.textContent = totalLic ? pctLic + '%' : '—';
  if (elSub) elSub.textContent = totalLic ? `${usadasLic.toLocaleString('pt-BR')} de ${totalLic.toLocaleString('pt-BR')}` : 'Sem licenças cadastradas';
  if (elBar) elBar.style.width = pctLic + '%';

  // ── Próximas renovações ──
  const renovList = document.getElementById('renovList');
  if (renovList) {
    const hoje = new Date(); hoje.setHours(0,0,0,0);
    const comVenc = licencas.filter(l => l.renovacao).map(l => {
      const dias = Math.round((new Date(l.renovacao + 'T00:00:00') - hoje) / 86400000);
      return { ...l, dias };
    }).sort((a, b) => a.dias - b.dias).slice(0, 5);
    if (!comVenc.length) {
      renovList.innerHTML = '<div style="color:var(--text-muted);font-size:12px">Nenhuma licença com data de renovação cadastrada.</div>';
    } else {
      renovList.innerHTML = comVenc.map(l => {
        const cls = l.dias < 0 ? 'dias-red' : l.dias <= 30 ? 'dias-red' : l.dias <= 60 ? 'dias-amber' : 'dias-green';
        const label = l.dias < 0 ? `Vencida há ${Math.abs(l.dias)}d` : l.dias === 0 ? 'Hoje!' : `${l.dias} dias`;
        return `<div class="renov-item">
          <div><div style="font-size:13px;font-weight:500">${l.nome}</div>
          <div style="font-size:12px;color:var(--text-muted)">${l.usadas || 0} / ${l.total || '—'} licenças</div></div>
          <span class="dias-badge ${cls}">${label}</span>
        </div>`;
      }).join('');
    }
  }
...
// ── Editor de softwares/custos do NOTEBOOK ──
function addSoftwareRowNb(nome = '', valor = '') {
  const row = document.createElement('div');
  row.className = 'nb-sw-row';
  row.style.cssText = 'display:flex;gap:6px;align-items:center';
  const nomeEsc = String(nome).replace(/"/g, '&quot;');
  const valEsc = (valor !== '' && valor != null) ? valor : '';
  row.innerHTML = `
    <input type="text" list="dl-software" placeholder="Software" value="${nomeEsc}" style="flex:1;min-width:0;border:0.5px solid var(--border);border-radius:var(--radius);padding:8px 10px;font-size:13px;background:var(--surface-1);color:var(--text-primary)">
    <input type="number" step="0.01" placeholder="R$/mês" value="${valEsc}" oninput="recalcCustoNb()" style="width:110px;border:0.5px solid var(--border);border-radius:var(--radius);padding:8px 10px;font-size:13px;background:var(--surface-1);color:var(--text-primary)">
    <button type="button" class="icon-btn danger" title="Remover" onclick="this.closest('.nb-sw-row').remove();recalcCustoNb()"><i class="ti ti-trash" style="font-size:15px"></i></button>`;
  document.getElementById('nb-sw-list').appendChild(row);
}
function recalcCustoNb() {
  let tot = 0;
  document.querySelectorAll('#nb-sw-list .nb-sw-row').forEach(r => {
    const v = parseFloat(r.querySelectorAll('input')[1].value);
    if (!isNaN(v)) tot += v;
  });
  document.getElementById('nb-custo').value = tot ? tot.toFixed(2) : '';
}
function collectSoftwaresNb() {
  const obj = {};
  document.querySelectorAll('#nb-sw-list .nb-sw-row').forEach(r => {
    const nome = r.querySelectorAll('input')[0].value.trim();
    const v = parseFloat(r.querySelectorAll('input')[1].value);
...
function loadSoftwareEditorNb(sw) {
  const list = document.getElementById('nb-sw-list');
  if (!list) return;
  list.innerHTML = '';
  if (sw) Object.entries(sw).forEach(([k, v]) => addSoftwareRowNb(k, v));
  recalcCustoNb();
}
...
// ============================================================
// MOVIMENTAÇÃO: ESTOQUE ↔ INVENTÁRIO
// ============================================================
function abrirEstoqueToInv(id) {
  const item = estoque.find(i => String(i.id) === String(id));
  if (!item) return;
  document.getElementById('etoi-id').value       = id;
  document.getElementById('etoi-nome').value     = item.produto || '';
  document.getElementById('etoi-codigo').value   = '';
  document.getElementById('etoi-cat').value      = '';
  document.getElementById('etoi-user').value     = '';
  document.getElementById('etoi-loc').value      = item.local_armaz || '';
  document.getElementById('etoi-status').value   = 'ok';
  // Preenche datalist de categoria
  const dl = document.getElementById('dl-cat-etoi');
  if (dl) {
    const cats = [...new Set(inventario.map(r => r.categoria).filter(Boolean))].sort();
    dl.innerHTML = cats.map(c => `<option>${c}</option>`).join('');
  }
  openModal('EstoqueToInv');
}

async function confirmarEstoqueToInv() {
  const estoqueId = document.getElementById('etoi-id').value;
  const codigo    = document.getElementById('etoi-codigo').value.trim();
  const cat       = document.getElementById('etoi-cat').value.trim();
  const nome      = document.getElementById('etoi-nome').value.trim();
  if (!codigo) { showToast('Informe o Código do ativo.', '#d03b3b'); return; }
  if (!cat)    { showToast('Informe a Categoria.', '#d03b3b'); return; }

  const item = estoque.find(i => String(i.id) === String(estoqueId));
  if (!item) return;

  // 1) Cria o ativo no inventário
  const payload = {
    codigo, nome: nome || item.produto,
    categoria:   cat,
    localizacao: document.getElementById('etoi-loc').value || 'A definir',
    usuario_resp: document.getElementById('etoi-user').value || null,
    serie:        item.numero_serie || null,
    patrimonio:   item.patrimonio || null,
    fabricante:   item.marca || null,
    observacoes:  item.observacoes || null,
    status:       document.getElementById('etoi-status').value,
    created_by:   currentUser.id,
  };
  const { data: novoAtivo, error: errAtivo } = await sb.from('ativos').insert(payload).select().single();
  if (errAtivo) { showToast('Erro ao criar ativo: ' + errAtivo.message, '#d03b3b'); return; }

  // 2) Registra no histórico do ativo com detalhe da origem
  await sb.from('historico').insert({
    ativo_id: novoAtivo.id, ativo_codigo: codigo, ativo_nome: nome || item.produto,
    tipo: 'Movimentação',
    descricao: `Transferido do estoque para o inventário por ${currentProfile.nome}.\nOrigem: estoque (produto: ${item.produto}${item.marca ? ', marca: ' + item.marca : ''}${item.numero_serie ? ', série: ' + item.numero_serie : ''}${item.local_armaz ? ', local: ' + item.local_armaz : ''}).`,
    responsavel: currentProfile.nome, created_by: currentUser.id,
  });

  // 3) Remove do estoque
  const { error: errEst } = await sb.from('estoque').delete().eq('id', estoqueId);
  if (errEst) { showToast('Ativo criado, mas erro ao remover do estoque: ' + errEst.message, '#d03b3b'); }

  closeModal('EstoqueToInv');
  await loadAtivos();
  await loadEstoque();
  showToast(`✓ "${nome || item.produto}" movido para o inventário.`);
}

async function devolverAoEstoque(ativoId) {
  const a = inventario.find(r => String(r.id) === String(ativoId));
  if (!a) return;
  if (!confirm(`Devolver "${a.nome}" ao estoque?\n\nO ativo será removido do inventário e criado no estoque.`)) return;

  // 1) Cria no estoque
  const { error: errEst } = await sb.from('estoque').insert({
    produto:      a.nome,
    marca:        a.fabricante || null,
    numero_serie: a.serie || null,
    patrimonio:   a.patrimonio || null,
    local_armaz:  a.localizacao || null,
    observacoes:  `Devolvido do inventário em ${new Date().toLocaleDateString('pt-BR')} por ${currentProfile.nome}. Código anterior: ${a.codigo}. Categoria: ${a.categoria || '—'}. Último usuário: ${a.usuario_resp || '—'}.${a.observacoes ? '\n' + a.observacoes : ''}`,
    created_by:   currentUser.id,
  });
  if (errEst) { showToast('Erro ao criar item no estoque: ' + errEst.message, '#d03b3b'); return; }

  // 2) Remove o histórico vinculado (necessário por foreign key)
  await sb.from('historico').delete().eq('ativo_id', ativoId);

  // 3) Remove o ativo do inventário
  const { error: errAtivo, data: delData } = await sb.from('ativos').delete().eq('id', ativoId).select();
  console.log('delete ativo result:', errAtivo, delData);
  if (errAtivo) {
    showToast('Erro ao remover do inventário: ' + errAtivo.message, '#d03b3b');
    await sb.from('estoque').delete().eq('produto', a.nome).eq('created_by', currentUser.id);
    return;
  }
  ...
  await loadAtivos();
  await loadEstoque();
  showToast(`✓ "${a.nome}" devolvido ao estoque.`);
}

async function loadEstoque() {
  try {
    const { data, error } = await sb.from('estoque').select('*').order('created_at', { ascending: false });
    if (error) throw error;
    estoque = data || [];
    applyEstoqueFilter();
  } catch (e) {
    console.warn('Estoque não carregado (tabela pode não existir ainda):', e.message);
    const body = document.getElementById('estoqueBody');
    if (body) body.innerHTML = `<tr><td colspan="7" style="text-align:center;padding:32px;color:var(--danger)">Tabela de estoque não encontrada. Execute o SQL de criação no Supabase.</td></tr>`;
  }
}

function applyEstoqueFilter() {
  const q = (document.getElementById('estoqueSearch')?.value || '').toLowerCase();
  const filtered = estoque.filter(i =>
    !q || (i.produto||'').toLowerCase().includes(q) ||
          ...
          (i.local_armaz||'').toLowerCase().includes(q)
  );
...
function renderEstoque(data) {
  ...
  const p = PERMS[currentProfile.role];
  if (!data.length) {
    body.innerHTML = `<tr><td colspan="7" style="text-align:center;padding:32px;color:var(--text-muted)">Nenhum item em estoque.</td></tr>`;
    return;
  }
  body.innerHTML = data.map(i => `
    <tr>
      <td style="font-weight:500">${i.produto}</td>
      <td>${i.marca || '—'}</td>
      <td style="color:var(--text-muted);font-size:12px">${i.numero_serie || '—'}</td>
      <td style="font-size:12px">${i.patrimonio || '—'}</td>
      <td style="color:var(--text-secondary)">${i.local_armaz || '—'}</td>
      <td style="color:var(--text-muted);font-size:12px">${i.observacoes || '—'}</td>
      <td><div class="row-actions">
        <button class="icon-btn" title="Mover para inventário" onclick="abrirEstoqueToInv('${i.id}')"><i class="ti ti-arrow-right" style="font-size:15px"></i></button>
        ${p.canEdit ? `<button class="icon-btn" title="Editar" onclick="editEstoque('${i.id}')"><i class="ti ti-pencil" style="font-size:15px"></i></button>` : ''}
        ${p.canDelete ? `<button class="icon-btn danger" title="Excluir" onclick="deleteEstoque('${i.id}','${(i.produto||'').replace(/'/g,'')}')"><i class="ti ti-trash" style="font-size:15px"></i></button>` : ''}
      </div></td>
    </tr>`).join('');
}
...
function editEstoque(id) {
  ...
  if (!item) return;
  editEstoqueId = id;
  document.getElementById('estoque-titulo').childNodes[0].nodeValue = 'Editar item de estoque ';
  document.getElementById('est-produto').value   = item.produto || '';
  document.getElementById('est-marca').value     = item.marca || '';
  document.getElementById('est-serie').value     = item.numero_serie || '';
  document.getElementById('est-patrimonio').value = item.patrimonio || '';
  document.getElementById('est-local').value     = item.local_armaz || '';
  document.getElementById('est-obs').value       = item.observacoes || '';
  openModal('Estoque');
}

function resetEstoqueModal() {
  editEstoqueId = null;
  document.getElementById('estoque-titulo').childNodes[0].nodeValue = 'Novo item de estoque ';
  ['est-produto','est-marca','est-serie','est-patrimonio','est-local','est-obs'].forEach(id => document.getElementById(id).value = '');
}

async function salvarEstoque() {
  const produto = document.getElementById('est-produto').value.trim();
  if (!produto) { showToast('Informe o nome do produto.', '#d03b3b'); return; }
  const payload = {
    produto,
    marca:        document.getElementById('est-marca').value || null,
    numero_serie: document.getElementById('est-serie').value || null,
    patrimonio:   document.getElementById('est-patrimonio').value || null,
    local_armaz:  document.getElementById('est-local').value || null,
    observacoes:  document.getElementById('est-obs').value || null,
  };
  if (editEstoqueId) {
    const { error } = await sb.from('estoque').update(payload).eq('id', editEstoqueId);
    if (error) { showToast('Erro ao salvar: ' + error.message, '#d03b3b'); return; }
  } else {
    payload.created_by = currentUser.id;
    const { error } = await sb.from('estoque').insert(payload);
    if (error) { showToast('Erro ao salvar: ' + error.message, '#d03b3b'); return; }
  }
  closeModal('Estoque');
  resetEstoqueModal();
  await loadEstoque();
  showToast(editEstoqueId ? 'Item atualizado.' : 'Item adicionado ao estoque.');
}

async function deleteEstoque(id, nome) {
...
// ============================================================
// CUSTO POR SETOR E CENTRO DE CUSTO
// ============================================================
function renderCustoPorGrupo(containerId, campo) {
  const el = document.getElementById(containerId);
  if (!el) return;
  const map = {};
  [...inventario, ...notebooksLocados].forEach(item => {
    const chave = item[campo] || 'Sem ' + (campo === 'setor' ? 'setor' : 'centro de custo');
    map[chave] = (map[chave] || 0) + (item.custo_mensal || 0);
  });
  const lista = Object.entries(map).filter(([,v]) => v > 0).sort((a, b) => b[1] - a[1]);
  if (!lista.length) { el.innerHTML = '<div style="color:var(--text-muted);font-size:13px;padding:8px 0">Nenhum custo cadastrado.</div>'; return; }
  const max = lista[0][1];
  el.innerHTML = lista.map(([nome, val]) => `
    <div style="margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:3px">
        <span>${nome}</span><span style="font-weight:500">${fmtMoeda(val)}</span>
      </div>
      <div class="progress-bar"><div class="progress-fill progress-blue" style="width:${Math.round(val/max*100)}%"></div></div>
    </div>`).join('');
}
...
function employeeCosts() {
  const map = {};
  const add = (item) => {
    const f = (item.usuario_resp && item.usuario_resp.trim()) ? item.usuario_resp.trim() : 'Sem responsável';
    if (!map[f]) map[f] = { nome: f, ativos: [], total: 0, softwares: {} };
    map[f].ativos.push(item);
    map[f].total += item.custo_mensal || 0;
    if (item.softwares) Object.entries(item.softwares).forEach(([k, v]) => { map[f].softwares[k] = (map[f].softwares[k] || 0) + v; });
  };
  inventario.forEach(add);
  notebooksLocados.forEach(add);
  return map;
}
...
function renderCustos() {
  ...
  const busca = (document.getElementById('custoBusca')?.value || '').toLowerCase().trim();
  ...
  renderCustoPorGrupo('custoSetor', 'setor');
  renderCustoPorGrupo('custoCC', 'centro_custo');
  ...
  if (!escolhido) {
    const filtrados = funcs.filter(f => !busca || f.nome.toLowerCase().includes(busca));
    cont.innerHTML = `<div class="card" style="overflow:auto">
      ...
      <table class="full-table">
        <thead><tr><th>Funcionário</th><th>Equipamentos</th><th>Custo mensal</th><th>Custo anual</th></tr></thead>
        <tbody>${filtrados.length ? filtrados.map(f => `
          ...
          </tr>`).join('') : `<tr><td colspan="4" style="text-align:center;padding:28px;color:var(--text-muted)">Nenhum colaborador encontrado para "${busca}".</td></tr>`}</tbody>
      </table>
    </div>`;
    return;
  }
  ...
  cont.innerHTML = `
    <div class="grid2">
      ...
      <div class="card">
        ...
        ${f.ativos.map(a => `<div class="license-row" style="padding:8px 0;${a.codigo ? 'cursor:pointer' : ''}" ${a.codigo ? `onclick="showDetalhe('${a.id}')"` : ''}>
          <div><div style="font-size:13px">${a.nome || [a.fabricante, a.modelo].filter(Boolean).join(' ') || 'Notebook'}</div><div style="font-size:11px;color:var(--text-muted)">${a.codigo ? `${a.codigo} · ${a.categoria}` : `Notebook locado · ${a.numero_serie || '—'}`}</div></div>
