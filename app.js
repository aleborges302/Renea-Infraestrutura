  ...
  body.innerHTML = keys.map(usuario => {
    ...
    const linhas = itens.map(r => `
      <tr onclick="showDetalhe('${r.id}')" style="cursor:pointer">
        ...
        <td style="color:var(--text-muted);font-size:12px">${r.patrimonio || '—'}</td>
        ...
        <td style="color:var(--text-secondary)">${r.centro_custo || '—'}</td>
        <td style="color:var(--text-secondary);font-size:12px">${r.numero_corporativo || '—'}</td>
    ...
    return `
      ...
      <tr id="${grpId}" class="grp-body">
        <td colspan="${cols}" style="padding:0">
          <table class="full-table" style="table-layout:fixed;width:100%">${linhas}</table>
        </td>
      </tr>`;
  }).join('');
